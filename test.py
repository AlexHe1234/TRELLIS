...
path = 'data/DeformingThings4D/animals/foxYSY_Jump/foxYSY_Jump.anime'

nf,nv,nt,v,f,off = anime_read(path)

v = np.concatenate([v[None],v+off],axis=0)[:24] #24,n,3

out_path = 'data/Trellis_Cache/raw_mesh'

for i in range(len(v)):
    trimesh.Trimesh(v[i], f).export(out_path + f'/foxYSY_Jump_{i:02d}.obj')
    break
